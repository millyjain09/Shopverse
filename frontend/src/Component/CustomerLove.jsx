...
// --- EXPANDED DATA (More reviews for smooth loop) ---
const reviews = [
  {
    id: 1,
    name: "Rohan Das",
...
const CustomerLove = () => {
  ...
  const settings = {
    dots: true,
    className: "center",
    centerMode: true, // ✅ Center wala card focus mein rahega
    infinite: true,
    centerPadding: "0px",
  ...
  .slick-center > div {
    ...
    z-index: 10;
      ...
      <div className='text-center mb-16'>
        ...
        <p className='text-teal-600 mt-2 font-medium'>See what our happy customers say about us</p>
      </div>

      <div className='max-w-7xl mx-auto px-0 md:px-4'>
        <Slider {...settings}>
          {reviews.map((review) => (
            // Wrapper Div for Padding (Taaki scale hone par kate nahi)
            <div key={review.id} className="py-10 px-2 outline-none"> 
              
              {/* --- YOUR CARD DESIGN (As requested) --- */}
              <div className='bg-white p-8 rounded-2xl shadow-xl border border-teal-100 h-full flex flex-col justify-between relative mx-2'>
                ...
                <div className='flex gap-1 mt-6 text-yellow-400 text-sm'>
                   {[...Array(5)].map((_, i) => (
                     <FaStar key={i} className={i < Math.floor(review.rating) ? "text-yellow-400" : "text-gray-300"} />
                   ))}
                </div>
